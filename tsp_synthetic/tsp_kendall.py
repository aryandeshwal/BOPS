...
def main():
    ...
    benchmark_index =  0 # int(sys.argv[2])
    for nruns in range(20):
        ...
        for num_iters in range(n_init, n_evals):
            ...
            scipy.io.savemat('bo_tsp.mat', {'theta_ls_matrix':tls_matrix, 'tls_matrix':theta_ls_matrix, 'theta':theta, 'dim':dim})
            eng = matlab.engine.start_matlab()
            eng.run_sdp_tsp(nargout=0)
            ...
            torch.save({'inputs_selected':train_x, 'outputs':train_y}, 'tsp_kendall_ts_sdp_benchmark_index_'+str(benchmark_index)+'_'+str(dim)+'_'+str(nruns)+'.pkl')
